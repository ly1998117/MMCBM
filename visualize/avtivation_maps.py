...
class CAMBase:
    ...
    def _upsample_and_post_process(self, acti_map, img_spatial):
...
class CAM(CAMBase):
    ...
    def compute_map(self, x, class_idx=None, layer_idx=-1, **kwargs):
        ...
        #     import pdb
        #     pdb.set_trace()
        #     raise RuntimeError("The model does not have the required hooks.")
        #
        acti = acti[layer_idx]
        b, c, *spatial = acti.shape

        if class_idx is None:
            class_idx = logits.max(1)[-1].reshape(-1, 1).expand(-1, b // logits.shape[0]).reshape(-1)
        ...
        output = torch.stack([output[i, j: j + 1] for i, j in enumerate(class_idx)], dim=0)
    ...
    def __call__(self, x, class_idx=None, layer_idx=-1, **kwargs):
        ...
        return self._upsample_and_post_process(acti_map, list(x.values())[0].shape[-2:])
